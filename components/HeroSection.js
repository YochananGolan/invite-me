...
const MobileHomepageHero = ({ onCreateEvent, onShowProcess }) => (
    ...
    <div className="mx-auto flex max-w-md flex-col items-center px-1 pb-6 pt-1 text-center">
      ...
      <div className="mt-5 w-full overflow-hidden rounded-3xl border border-white/15 bg-white/[0.055] p-4 text-right shadow-[0_14px_46px_rgba(0,0,0,0.38)] backdrop-blur-xl">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <p className="text-xs font-bold text-slate-400">כרטיס הדגמה</p>
            <h2 className="mt-1 text-xl font-black text-white">חתונת דניאל ומיכל</h2>
          </div>
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-emerald-300/30 bg-emerald-400/10 text-2xl">
            💐
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            <div key={label} className={`rounded-2xl border px-2 py-3 text-center ${cardClass}`}>
              <div className={`text-2xl font-black leading-none tabular-nums ${valueClass}`}>{value}</div>
              <div className="mt-1 text-[11px] font-bold text-slate-300">{label}</div>
            </div>
          ))}
        </div>
      </div>
...
export default forwardRef(function HeroSection({ onStart, onPressCreateEvent, session }, ref) {
  ...
  return (
    <section className="relative z-10 overflow-visible px-4 pb-10 pt-12 text-slate-100 sm:px-6 lg:px-12 lg:pb-14 lg:pt-16">
      <MobileHomepageHero onCreateEvent={handleCreateNewEvent} onShowProcess={() => onStart?.()} />
