...
export async function sendEventWhatsAppInvites(supabase, options) {
  ...
  for (const guest of guests || []) {
    ...
    if (!normalizedPhone || seenGuestPhones.has(normalizedPhone)) continue;
    seenGuestIds.add(guest.id);
    seenGuestPhones.add(normalizedPhone);
    uniqueGuests.push(guest);
  }
  const guestsWithPhone = uniqueGuests;
  ...
  if (queuedCount > 0 && !skipMessageCountIncrement) {
    try {
      const sentGuestIds = results
        .filter((result) => result.ok && result.guestId)
