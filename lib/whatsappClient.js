...
async function isGreenApiParticipantInGroup({ greenApi, groupId, participantChatId }) {
  try {
    ...
    if (!groupDataResponse.ok || !Array.isArray(groupData?.participants)) {
      return { ok: false, data: groupData };
    }

    const isMember = groupData.participants.some((participant) => {
      const id = String(participant?.id || participant?.chatId || '').trim();
      return id === participantChatId;
    });

    return { ok: true, isMember, data: groupData };
  } catch (err) {
...
export async function findWhatsAppGroupByName({ groupName, phones = [] }) {
  let greenApi;
  try {
    greenApi = ensureGreenApiConfigured();
  } catch (err) {
  ...
  const expectedParticipantIds = new Set((phones || []).map(toGreenApiChatId).filter(Boolean));
    ...
    for (const group of candidateGroups) {
      ...
      const participantIds = new Set(
        (Array.isArray(groupData?.participants) ? groupData.participants : [])
          .map((participant) => String(participant?.id || participant?.chatId || '').trim())
          .filter(Boolean),
      );
      const matchedByName =
        subject === cleanName ||
        (subject && (subject.includes(cleanName) || cleanName.includes(subject)));
      const matchedByParticipant =
        expectedParticipantIds.size > 0 &&
        Array.from(expectedParticipantIds).some((participantId) => participantIds.has(participantId));

      if (matchedByName || matchedByParticipant) {
        return {
          ok: true,
...
export async function addWhatsAppGroupParticipant({ groupId, phone }) {
  ...
  try {
    ...
    if (!response.ok || !added) {
      const membershipCheck = await isGreenApiParticipantInGroup({
        greenApi,
        groupId: cleanGroupId,
        participantChatId,
      });

      if (membershipCheck.ok && membershipCheck.isMember) {
        return {
          ok: true,
          status: 'already_in_group',
          error: null,
          data: {
            addParticipantResponse: responseBody,
            membershipCheck: 'participant_already_exists',
          },
          participantChatId,
          durationMs: Date.now() - startedAt,
        };
      ...
      return {
        ...
        error: getGreenApiError(
          responseBody,
          'Green API did not add the participant and did not return a detailed reason',
        ),
        data: {
          addParticipantResponse: responseBody,
          membershipCheck: membershipCheck.ok
            ? 'participant_not_in_group_after_failed_add'
            : membershipCheck.error || membershipCheck.data || 'membership_check_failed',
        },
        participantChatId,
        durationMs: Date.now() - startedAt,
      };
