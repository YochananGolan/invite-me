...
export const isEventWizardDraft = (record) => {
  ...
  // אירוע שתאריכו עבר הסתיים — גם אם עמודת status עדיין 'draft'. אחרת הוא לא עובר לארכיון.
  const rawDate = getRawEventDateFromDetails(getEventDetailsFromRecord(record));
  if (rawDate && isEventDateBeforeToday(rawDate)) return false;
...
/**
 * שלבי האשף (סוג אירוע וכו') נפתחים רק כשיש מסלול ששולם — לא לפי שאריות localStorage
 * ולא לפי אירוע פעיל בלבד, כדי שרשומת אירוע ללא מסלול לא תפתח את האשף בלי תשלום.
 */
export const hasPaidWizardAccess = ({
  isCurrentEventActive = false,
  isPaidWizardInProgress: paidWizard = false,
  plan = null,
  eventWizardStarted = false,
} = {}) => {
  if (paidWizard) return true;
  if (!plan) return false;
  return Boolean(isCurrentEventActive || eventWizardStarted);
};
