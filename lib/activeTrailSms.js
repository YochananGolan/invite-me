  ...
  for (const guest of guests) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`ActiveTrail SMS error for ${phone}:`, rawErrorText);
        let normalizedError = `API error: ${response.status}`;
        try {
          const parsed = rawErrorText && rawErrorText.trim().startsWith('{')
            ? JSON.parse(rawErrorText)
            : null;
          if (parsed && (parsed.message || parsed.error)) {
            normalizedError = parsed.message || parsed.error;
          } else if (rawErrorText) {
            normalizedError = rawErrorText;
          }
        } catch (_) {
          if (rawErrorText) normalizedError = rawErrorText;
        }
        errors.push({ guest, status: response.status, error: normalizedError });
      } else {
