...
export async function sendEventWhatsAppInvites(supabase, options) {
  ...
  const greenGapMs =
    provider === 'greenapi'
      ? Math.max(
          ...
          Math.floor(
            Number(
              process.env.WHATSAPP_GREENAPI_SEND_GAP_MS !== undefined
                ? process.env.WHATSAPP_GREENAPI_SEND_GAP_MS
                : GREENAPI_DEFAULT_GAP_MS,
            ),
          ),
        )
      : 0;
  ...
  if (sentCount > 0 && !skipMessageCountIncrement) {
    try {
